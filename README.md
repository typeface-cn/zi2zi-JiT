...
- **内容编码器** — 一个 CNN，用于捕获输入字符的结构布局，改编自 [FontDiffuser](https://arxiv.org/abs/2312.12142)。
- **风格编码器** — 一个 CNN，用于从目标字体的参考字形中提取风格特征。
- **多源上下文混合** — 不同于原始 JiT 中仅基于单个类别标记进行条件化，字体、风格和内容嵌入被拼接为统一的条件序列。
...
使用 LoRA 在单 GPU 上微调预训练模型。以下示例使用 JiT-B/16，batch size 为 16，约需 4 GB 显存：
...
| 参数 | 说明 |
|---|---|
| `--num_fonts`、`--num_chars` | 与预训练模型的嵌入维度绑定。除非从头预训练，否则请勿更改。 |
